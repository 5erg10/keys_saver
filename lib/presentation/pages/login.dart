...
class LoginState extends ConsumerState<Login> {
  ...
  void initState() {
    super.initState();
    ref.read(credentialsProvider.notifier).recoverCredentials();
  ...
  Widget build(BuildContext context) {
    ...
    if (!credentialsPassed && userCredentials != null) {
      // comprueba si el usuario ha definido ya una provate key para la encriptacion
      if (userCredentials.passKey!.isEmpty) {
        setState(() {
          showCreatePassForm = false;
          showCreatePrivateKey = true;
        });
      } else if (!haveBiometrics) {
        setState(() {
          showCreatePrivateKey = false;
          showCreatePassForm = true;
        });
      } else {
    ...
    void onFormSubmit(String user, String passW) {
     ...
     setState(() {
      ...
      if (userCredentials!.user!.isEmpty) {
        ref.read(credentialsProvider.notifier).saveUserCredentials(userCredentials.. user = user.. passW = passW);
      } else {
        ref.read(credentialsProvider.notifier).saveUserCredentials(userCredentials.. passW = passW);
      }
      context.push('/');
    }

    void onPrivKeySubmit(String privKey) {
      ref.read(credentialsProvider.notifier).saveUserCredentials(userCredentials!.. passKey = privKey);
    }

    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: const Text('Keys Saver').boldSubString('Keys', Theme.of(context).textTheme.bodyMedium!),
